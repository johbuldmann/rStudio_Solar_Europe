...
solar_europe_de <- read_delim(file = path2,delim = ",") %>% # Rohdaten eingelesen
  ...
  mutate(global_radiation = radiation_direct_horizontal + radiation_diffuse_horizontal,
         solar_watt=global_radiation * ((-0.583 * temperature + 115)/100)) %>%
...
durchschnitt("2015-01-01 06:00:00","2015-12-31 24:00:00","DE11")
...
m2 <- function(kwhy, efficency = 0.2, startyear, endyear, selected_country){
  ...
  solar_europe_de_nuts %>%
    ...
    summarise(
      yieldm2 = sum(solar_watt) / years * efficency /1000,
      m = kwhy/yieldm2
           )
}


m2(kwhy = 4000,startyear = 2015, endyear = 2015, selected_country = "DE11") # 
...
kwhyield(startyear = 2015, endyear = 2015,efficency = 0.2, selected_country = "DE11", m2 = 70) # 
